...
// */
//package com.techindiana.oakstudiotv.pause;
//
...
//   * @param deserializers Used to deserialize {@link com.google.android.exoplayer2.offline.DownloadAction}s. If empty, {@link
//   *     com.google.android.exoplayer2.offline.DownloadAction#getDefaultDeserializers()} is used instead.
//   */
...
//    this.deserializers =
//        deserializers.length > 0 ? deserializers : com.google.android.exoplayer2.offline.DownloadAction.getDefaultDeserializers();
...
//   * #handleAction(com.google.android.exoplayer2.offline.DownloadAction)}.
//   *
...
//    com.google.android.exoplayer2.offline.DownloadAction action = com.google.android.exoplayer2.offline.DownloadAction.deserializeFromStream(deserializers, input);
...
//   * @return The id of the newly created task.
//   */
//  public int handleAction(com.google.android.exoplayer2.offline.DownloadAction action) {
//    Assertions.checkState(!released);
//    Task task = addTaskForAction(action);
//    if (initialized) {
//      saveActions();
//      maybeStartTasks();
//      if (task.currentState == TaskState.STATE_QUEUED) {
...
//  }
//
//  private Task addTaskForAction(com.google.android.exoplayer2.offline.DownloadAction action) {
...
//      }
//
//      com.google.android.exoplayer2.offline.DownloadAction action = task.action;
...
//    fileIOHandler.post(
//        () -> {
//          com.google.android.exoplayer2.offline.DownloadAction[] loadedActions;
//          try {
...
//            loadedActions = new com.google.android.exoplayer2.offline.DownloadAction[0];
//          }
//          final com.google.android.exoplayer2.offline.DownloadAction[] actions = loadedActions;
//          handler.post(
//              () -> {
//                if (released) {
//                  return;
...
//                tasks.clear();
//                for (com.google.android.exoplayer2.offline.DownloadAction action : actions) {
...
//      return;
//    }
//    final com.google.android.exoplayer2.offline.DownloadAction[] actions = new com.google.android.exoplayer2.offline.DownloadAction[tasks.size()];
//    for (int i = 0; i < tasks.size(); i++) {
...
//    private TaskState(
//        int taskId,
//        com.google.android.exoplayer2.offline.DownloadAction action,
//        @State int state,
...
//      this.action = action;
//      this.currentState = TaskState.STATE_QUEUED;
//      this.minRetryCount = minRetryCount;
//    }
//
...
//      return currentState == TaskState.STATE_FAILED
//          || currentState == TaskState.STATE_COMPLETED
//          || currentState == TaskState.STATE_CANCELED;
//    }
//
//    /** Returns whether the task is started. */
//    public boolean isActive() {
//      return currentState == STATE_QUEUED_CANCELING
//          || currentState == TaskState.STATE_STARTED
...
//      switch (currentState) {
//        case STATE_QUEUED_CANCELING:
//          return TaskState.STATE_QUEUED;
//        case STATE_STARTED_CANCELING:
//        case STATE_STARTED_STOPPING:
//          return TaskState.STATE_STARTED;
//        case TaskState.STATE_QUEUED:
//        case TaskState.STATE_STARTED:
//        case TaskState.STATE_COMPLETED:
//        case TaskState.STATE_CANCELED:
...
//
//    private void start() {
//      if (changeStateAndNotify(TaskState.STATE_QUEUED, TaskState.STATE_STARTED)) {
...
//      return currentState == TaskState.STATE_QUEUED;
//    }
//
//    private void cancel() {
//      if (changeStateAndNotify(TaskState.STATE_QUEUED, STATE_QUEUED_CANCELING)) {
//        downloadManager.handler.post(
//            () -> changeStateAndNotify(STATE_QUEUED_CANCELING, TaskState.STATE_CANCELED));
//      } else if (changeStateAndNotify(TaskState.STATE_STARTED, STATE_STARTED_CANCELING)) {
//        cancelDownload();
//      }
//    }
//
//    private void stop() {
//      if (changeStateAndNotify(TaskState.STATE_STARTED, STATE_STARTED_STOPPING)) {
//        logd("Stopping", this);
//        cancelDownload();
//      }
//    }
...
//                errorCount = 0;
//              }
//              if (currentState != TaskState.STATE_STARTED || ++errorCount > minRetryCount) {
//                throw e;
//              }
...
//                    TaskState.STATE_STARTED, finalError != null ? TaskState.STATE_FAILED : TaskState.STATE_COMPLETED, finalError)
//                || changeStateAndNotify(STATE_STARTED_CANCELING, TaskState.STATE_CANCELED)
//                || changeStateAndNotify(STATE_STARTED_STOPPING, TaskState.STATE_QUEUED)) {
//              return;
//            }
//            throw new IllegalStateException();
//          });
